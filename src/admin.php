		...
		wp_add_dashboard_widget(
			...
			function (): void {
				...
				<p>
					...
					printf( esc_html__( '%1$s images · %2$s of %3$s files compressed (full size and every image size).', 'grist' ), esc_html( number_format_i18n( $t['images'] ) ), esc_html( number_format_i18n( $t['done'] ) ), esc_html( number_format_i18n( $t['files'] ) ) );
					?>
				</p>
				<?php if ( $t['before'] > 0 ) : ?>
					<p><strong>
						...
						printf( esc_html__( 'Saved %1$s (%2$d%%) compared with WordPress on its own.', 'grist' ), esc_html( size_format( max( 0, $t['saved'] ), 1 ) ), (int) round( 100 * $t['saved'] / $t['before'] ) );
						?>
					</strong></p>
				<?php endif; ?>
				<?php if ( $t['pending'] > 0 ) : ?>
					<p>
						...
						?>
						<a href="<?php echo esc_url( admin_url( 'upload.php?mode=list&grist=pending' ) ); ?>"><?php esc_html_e( 'Show them', 'grist' ); ?></a>,
						<?php esc_html_e( 'then select all and use Bulk actions → Compress.', 'grist' ); ?>
					</p>
				<?php endif; ?>
				<p class="description">
					<?php
